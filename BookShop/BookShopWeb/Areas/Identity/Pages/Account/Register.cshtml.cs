...
namespace WebApplication1.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            {
                Roles = _roleManager.Roles.Select(r => r.Name).Select(s => new SelectListItem { Text = s, Value = s }),
                Companies=_unitOfWork.Company.GetAll().Select(s=>new SelectListItem { Text=s.Name,Value=s.Id.ToString()})
            };
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (Input.Role == Role.Company.ToString())
                {
                ...
                if (result.Succeeded)
                {
                    ...
                    if(Input.Role==null)
                    {
                        await _userManager.AddToRoleAsync(user, Role.Individual.ToString());
                    }
                    else
                    {
